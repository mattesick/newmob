...
function addUser($email, $password, $firstName, $lastName, $role)
{
    ...
    $stmt = $conn->prepare('INSERT INTO user (email, password, firstName, lastName, role, active) VALUES (?, ?, ?, ?, ?, ?)');
    //this is where we bind all informtaion to our preperd query and then we execute it.
    $stmt->bind_param('sssssi', $email, $hash, $firstName, $lastName, $role, $active);
    $stmt->execute();
