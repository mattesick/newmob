...
class Engine extends ModelBase
{
    ...
    public function addUser($email, $password, $firstName, $lastName, $role)
    {
        ...
        $active = 0;
        $this->provider->query('INSERT INTO user (email, password, firstName, lastName, role, active) VALUES (?, ?, ?, ?, ?, ?)', $email, $hash, $firstName, $lastName, $role, $active);
        $uid = getUserWithEmail($email)["id"];
        $hash = makeId();
        $host = $this->provider->properties["host"];
        $html = "
<!DOCTYPE html>
